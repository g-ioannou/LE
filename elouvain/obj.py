...
class Graph:
    ...
    def update_based_on_partition(self, new_partition) -> Graph:
        """
    ...
    def to_nx(self) -> nx.Graph:
        ...
        node_attr = self.nodes.select("id", "partition").toPandas().set_index("id").to_dict("index")
    ...
    def induced_subgraph(self, starting_node: DataFrame, depth: int) -> Graph:
        """
        ...
        subgraph_nodes = self.ST.reload_df(subgraph_nodes, "subgraph_nodes")
        ...
        for depth in range(1, depth):  # pylint: disable=unused-variable
            ...
            subgraph_nodes = (
                subgraph_nodes.union(subgraph_edges.select("dst").withColumnRenamed("dst", "id"))
                .distinct()
                .repartition(8, "id")
                .cache()
            )
        ...
        subgraph_nodes = self.ST.spark.sql(
            "SELECT * FROM nodes_df WHERE nodes_df.id IN (SELECT * FROM subgraph_nodes)"
        ).cache()

        subgraph_nodes = self.ST.reload_df(subgraph_nodes, "subgraph_nodes", 8)
        subgraph_edges = self.ST.reload_df(subgraph_edges, "subgraph_edges", 8)
    ...
    def get_random_node(self) -> int:
        ...
        random_node = self.nodes.rdd.takeSample(withReplacement=False, num=1, seed=0)
